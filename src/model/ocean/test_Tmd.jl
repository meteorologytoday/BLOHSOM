...
pplan = Tmd.ParallelPlan(gf.Ny)

master = Tmd.TmdMaster(
    pplan = pplan,
    gf                    = gf,
    Δt                    = Δt,
    substeps              = 8,
...
# Setting up recorder
complete_varlist = Tmd.getCompleteVariableList(master)
varlist = []
...
coord_varlist = Tmd.getCoordinateVariable(master)
cvarlist = []
...
master.state.h_ML .= 7.0

master.state.S_ML .= 35.0
master.state.S .= 35.0

master.state.T .= 10.0
master.state.T_ML .= 10.0
master.state.T_ML .= 20 .+ 10.0 * sin.((master.env.gi.c_lon )) .* sin.(master.env.gi.c_lat)
#master.state.T[1, :, :] .= master.state.T_ML


#master.state.swflx .= -1000.0
master.state.h_ML  .= master.state.h_ML
#master.state.Φ .= 1.0 * 10.0 * sin.((master.env.gi.c_lon )) .* sin.(master.env.gi.c_lat * 2)
σ = 100e3 * 10.0
#master.state.u_c[1, :, :] .= 1.0

master.state.u_U[1, :, :] .= 1 .* cos.(2*master.env.gi.c_lat)#* sin.((master.env.gi.c_lon )) .* cos.(master.env.gi.c_lat*2) #.* exp.( - (master.env.gi.c_y / σ).^2.0 / 2.0 )

Tmd.initialization!(master)

recorder = RecordTool.Recorder(
    Dict(
        "NX"      => master.env.NX,
        "Nyp1"    => master.env.Ny+1,
        "Nz_p1"  => master.env.Nz+1,
        "Nx"      => master.env.Nx,
        "Ny"      => master.env.Ny,
        "Nz"    => master.env.Nz,
        "z_bnd" => length(master.env.z_bnd),
    ), varlist, Dict(),
    other_vars = cvarlist
)
...
function integrateT()
    s = 0.0
    for k=1:master.env.Nz
        s += sum(gi.dσ .* view(master.state.T, k, :, :))
    end
    return s / sum_dσ / master.env.Nz
end
...
@time for step=1:run_days
    ...
    for substep = 1:master.env.substeps
        Tmd.stepModel!(master)
    end
    RecordTool.record!(recorder)
    RecordTool.avgAndOutput!(recorder)
